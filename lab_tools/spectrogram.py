...
def save_signal(signal_data, time_data, file_path):
    data = {
        "Time [sec]": time_data,
        "Signal Raw [uV]": signal_data,
    }
...
def calculate_frequency_band_intensity(
    ...
    smoothed_band_intensity = np.convolve(band_intensity, np.ones(window_size) / window_size, mode='valid')

    return band_intensity, smoothed_band_intensity


def plot_all_band_intensity(
                time_array, frequency_array, analysis_matrix, method="STFT", integration_method="simps"):
    # 各帯域の強度を計算
    _, gamma_smothed = calculate_frequency_band_intensity(frequency_array, analysis_matrix, (30, 36), method=method, integration_method=integration_method)
    _, beta_smothed = calculate_frequency_band_intensity(frequency_array, analysis_matrix, (15, 30), method=method, integration_method=integration_method)
    _, alpha_smothed = calculate_frequency_band_intensity(frequency_array, analysis_matrix, (8, 12), method=method, integration_method=integration_method)
    _, theta_smothed = calculate_frequency_band_intensity(frequency_array, analysis_matrix, (4, 8), method=method, integration_method=integration_method)
    _, delta_smothed = calculate_frequency_band_intensity(frequency_array, analysis_matrix, (0, 4), method=method, integration_method=integration_method)
    ...
    plt.stackplot(
        ...
        theta_normalized,
        delta_normalized,
        labels=["Gamma (30-36 Hz)", "Beta (15-30 Hz)", "Alpha (8-12 Hz)", "Theta (4-8 Hz)", "Delta (0-4 Hz)"],
        colors=["#FF6347", "#FFD700", "#ADFF2F", "#87CEEB", "#6A5ACD"]
    )
    plt.xlabel("Time [sec]")
...
def export_arguments_to_yaml(
        ...
        integration_method, segment_length, overlap,
    ...
    arguments = {
        ...
        "all_band_intensity_analysis": {
            "smoothed_window_size_for_stft": 10,
            "smoothed_window_size_for_wavelet": 5000
        }
    }
...
def generate_spectrogram_and_signal_plot(
    ...
    # 信号プロットの保存
    signal_data_path = os.path.join(output_dir, "signal_data.csv")
    save_signal(signal_data, time_array, signal_data_path)
    ...
    if analysis_method == "Short-Time Fourier Transform":
        ...
        plt.figure(dpi=200)
        plot_all_band_intensity(times, frequencies, amplitude, method="STFT", integration_method=integration_method)
        ...
        plt.figure(dpi=200)
        plot_all_band_intensity(time_array, np.arange(max_frequency), cwt_matrix, method="CWT", integration_method=integration_method)
        plot_frequency_band_intensity_path = os.path.join(output_dir, "band_intensity.png")
        plt.savefig(plot_frequency_band_intensity_path)
    ...
    return spectrogram_plot_path, plot_frequency_band_intensity_path, ziped_file
